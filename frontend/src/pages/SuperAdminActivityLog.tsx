...
/** Superadmin view: all activity logs from all organizations. */
export function SuperAdminActivityLog() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 md:p-8">
      <h1 className="text-2xl font-bold text-slate-800 mb-2">Activity Log</h1>
      <p className="text-slate-500 mb-6">All actions across all organizations (CREATE, UPDATE, DELETE on Companies & Contacts).</p>
      {loading && !list.length ? (
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase">Org ID</th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {list.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-600">{row.organization_id ?? '—'}</td>
